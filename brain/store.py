...
def connect(db_path: str | os.PathLike[str] | None = None) -> sqlite3.Connection:
    ...
    path = Path(db_path or os.environ.get("BRAIN_DB") or DEFAULT_DB)
...
def upsert(
    ...
    *,
    ...
    needs_review = 1 if res.action == "review" else 0
    conn.execute(
        "INSERT OR REPLACE INTO entities "
        "(id, type, natural_key, title, summary, data, source, needs_review, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (res.key, type_, res.key, title, summary, payload, source, needs_review, now, now),
    )
    conn.commit()
    return res, res.key
